...
function init(){
    decode_workspace=/mnt/NFS1/zhangyanchao/src_mul_channel/multi_channel_${data_date}_${data_part_id}/decode_workspace_xdzj
...
function main(){
    ...
    sleep 60s
    cat $decode_workspace/*.out > $decode_workspace/../decode_xdzj.out
...
decode_script=${cur_dir}/decode_offline/start_decode_offline_xdzj.sh
